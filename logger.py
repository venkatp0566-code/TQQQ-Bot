...
def save_bot_state(state_dict):
    """Persists bot _state to DB so it survives restarts."""
    import json
    conn = sqlite3.connect(config.DATABASE_FILE)
    c = conn.cursor()
    c.execute("""
...
def load_bot_state():
    """Loads persisted bot _state from DB. Returns None if not found."""
    import json
    try:
        conn = sqlite3.connect(config.DATABASE_FILE)
        c = conn.cursor()
        c.execute("SELECT state_json FROM bot_state WHERE id = 1")
        row = c.fetchone()
        conn.close()
        if row:
