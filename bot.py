...
def job_signal_check():
    ...
    try:
        ...
        actual_alloc = min((tqqq_val / total_val) if total_val > 0 else 0, 1.0)
        ...
        logger.save_bot_state(_state)  # ← persist after every signal check
        # ── 8. Save to database ───────────────────────────────────────────────
        logger.save_signal(
...
def job_gap_guard():
    ...
    try:
        ...
        for order in orders_placed:
            ...
            qty        = float(order.qty)
            current_positions = orders.get_positions(client)
            if order.symbol in current_positions:
                fill_price = current_positions[order.symbol]["current_price"]
            else:
...
def startup():
    ...
    saved_state = logger.load_bot_state()
    if saved_state:
        _state["atr_extreme_mode"] = saved_state.get("atr_extreme_mode", False)
        _state["cb_state"]         = saved_state.get("cb_state", "OK")
        _state["last_regime"]      = saved_state.get("last_regime")
        _state["pending_buy"]      = saved_state.get("pending_buy", False)
        _state["pending_alloc"]    = saved_state.get("pending_alloc", 0.0)
        _state["pending_regime"]   = saved_state.get("pending_regime")
        logger.log_info("startup",
            f"State restored: pending_buy={_state['pending_buy']} "
            f"regime={_state['last_regime']} "
            f"alloc={_state['pending_alloc']:.1%}")
    else:
        logger.log_info("startup", "No saved state found — starting fresh")
